...
export class SidebarComponent {
  ...
  collapsed = signal(window.innerWidth < 900);
  mobileOpen = signal(false);

  @HostListener('window:resize')
  onResize() {
    this.collapsed.set(window.innerWidth < 900);
    if (window.innerWidth >= 900) this.mobileOpen.set(false);
  }
  ...
  constructor() {
    ...
    this.systemItems = [
      {
        label: 'UI.u_user_management',
        icon: this.icon('users'),
