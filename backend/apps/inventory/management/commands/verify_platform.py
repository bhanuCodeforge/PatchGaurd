...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 5. TLS Certificate Check (Sec 13.7)
        self.stdout.write("5. TLS Certificates... ", ending="")
        try:
             # Check a known pem file defined in settings
             from django.conf import settings
             cert_path = getattr(settings, "CERT_PATH", "/etc/nginx/ssl/server.crt")
             if os.path.exists(cert_path):
                 import subprocess
                 # openssl x509 -enddate -noout -in server.crt
                 res = subprocess.run(["openssl", "x509", "-enddate", "-noout", "-in", cert_path], capture_output=True, text=True)
                 if res.returncode == 0:
                     self.stdout.write(self.style.SUCCESS(f"OK ({res.stdout.strip()})"))
                 else:
                     self.stdout.write(self.style.WARNING("EXISTS BUT FAILED TO PARSE"))
             else:
